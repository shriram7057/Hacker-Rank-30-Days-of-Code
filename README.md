<p align="center">
  <img src="https://github.com/shriram7057/shriram7057/blob/main/HR%2030%20Days%20of%20Code%20Badge.png?raw=true" width="140" />
  <img src="https://github.com/shriram7057/shriram7057/blob/main/HR%20logo%20GH.jpeg?raw=true" width="280"/>
</p>
<h1 align="center">🚀 HackerRank – 30 Days of Code</h1>



<p align="center">
A complete collection of solutions for the <b>HackerRank 30 Days of Code</b> challenge, implemented using <b>C++</b> and <b>Python</b>.
</p>

<hr/>

<h2>📌 Project Overview</h2>
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr>
    <th align="left">Challenge Name</th>
    <td>HackerRank – 30 Days of Code</td>
  </tr>
  <tr>
    <th align="left">Description</th>
    <td>
      A 30-day coding challenge designed to strengthen problem-solving skills,
      data structures, algorithms, and core programming concepts.
    </td>
  </tr>
  <tr>
    <th align="left">Languages Used</th>
    <td>C++, Python</td>
  </tr>
  <tr>
    <th align="left">Difficulty Level</th>
    <td>Beginner to Intermediate</td>
  </tr>
</table>

<hr/>

<h2>🧠 Concepts Covered</h2>
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr>
    <th>Category</th>
    <th>Topics</th>
  </tr>
  <tr>
    <td>Basics</td>
    <td>Input/Output, Data Types, Operators, Conditional Statements</td>
  </tr>
  <tr>
    <td>Control Flow</td>
    <td>Loops, Recursion, Conditional Logic</td>
  </tr>
  <tr>
    <td>Data Structures</td>
    <td>Arrays, 2D Arrays, Linked Lists, Binary Search Trees</td>
  </tr>
  <tr>
    <td>Algorithms</td>
    <td>Sorting, Time Complexity Analysis</td>
  </tr>
  <tr>
    <td>Advanced Topics</td>
    <td>Regex, Bitwise Operations, Testing</td>
  </tr>
</table>

<hr/>

## 🧑‍💻𝗣𝗿𝗶𝗺𝗮𝗿𝘆 𝗟𝗮𝗻𝗴𝘂𝗮𝗴𝗲𝘀

<table>
  <tr>
    <td align="center" width="160">
      <img src="https://skillicons.dev/icons?i=c" width="48"/><br/>
      <b>C</b>
    </td>
    <td align="center" width="160">
      <img src="https://skillicons.dev/icons?i=cpp" width="48"/><br/>
      <b>C++</b>
    </td>
    <td align="center" width="160">
      <img src="https://skillicons.dev/icons?i=java" width="48"/><br/>
      <b>Java</b>
    </td>
    <td align="center" width="160">
      <img src="https://skillicons.dev/icons?i=python" width="48"/><br/>
      <b>Python</b>
    </td>
  </tr>
</table>

<h2>📁 Repository Structure</h2>
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr>
    <th>Folder / File</th>
    <th>Description</th>
  </tr>
  <tr>
    <td><code>Hacker-Rank-30 Days of Code/</code></td>
    <td>Main project directory containing all solutions</td>
  </tr>
  <tr>
    <td><code>Day_00.cpp</code> – <code>Day_11_2D_Arrays.cpp</code></td>
    <td>C++ solutions for early challenge days</td>
  </tr>
  <tr>
    <td><code>Day 20 Sorting.py</code></td>
    <td>Python implementation of sorting algorithms</td>
  </tr>
  <tr>
    <td><code>Day 22–23 BST</code></td>
    <td>Binary Search Tree problems in C++</td>
  </tr>
  <tr>
    <td><code>Day 24–29 *.py</code></td>
    <td>Advanced Python challenges including regex and bitwise AND</td>
  </tr>
</table>

<hr/>

<h2>🛠️ Tech Stack</h2>
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr>
    <th>Technology</th>
    <th>Usage</th>
  </tr>
  <tr>
    <td>C++</td>
    <td>Core algorithmic and data structure problems</td>
  </tr>
  <tr>
    <td>Python</td>
    <td>High-level logic, regex, testing, bitwise operations</td>
  </tr>
  <tr>
    <td>HackerRank</td>
    <td>Problem source and evaluation platform</td>
  </tr>
</table>

<hr/>

<h2>▶️ How to Run the Code</h2>
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr>
    <th>Language</th>
    <th>Command</th>
  </tr>
  <tr>
    <td>C++</td>
    <td>
      <pre>g++ filename.cpp -o output
./output</pre>
    </td>
  </tr>
  <tr>
    <td>Python</td>
    <td>
      <pre>python filename.py</pre>
    </td>
  </tr>
</table>

<hr/>

<h2>🎯 Learning Outcomes</h2>
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr>
    <td>✔ Improved problem-solving skills</td>
  </tr>
  <tr>
    <td>✔ Strong understanding of data structures</td>
  </tr>
  <tr>
    <td>✔ Hands-on experience with C++ and Python</td>
  </tr>
  <tr>
    <td>✔ Preparation for technical interviews</td>
  </tr>
</table>

<hr/>

<h2>📈 Future Enhancements</h2>
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr>
    <td>Add detailed explanations for each solution</td>
  </tr>
  <tr>
    <td>Include time and space complexity analysis</td>
  </tr>
  <tr>
    <td>Add links to corresponding HackerRank problems</td>
  </tr>
</table>

<hr/>

<h2>📜 License</h2>
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr>
    <td>
      This project is open-source and intended for educational purposes only.
    </td>
  </tr>
</table>

<hr/>

<h2>👤 Author</h2>
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <tr>
    <th>Name</th>
    <td>Your Name</td>
  </tr>
  <tr>
    <th>Platform</th>
    <td>HackerRank</td>
  </tr>
  <tr>
    <th>Purpose</th>
    <td>Learning, practice, and interview preparation</td>
  </tr>
</table>
<h3>🤝 Connect with Me</h3>

<table border="1" cellpadding="10" cellspacing="0" width="100%">
  <tr>
    <th align="center">Platform</th>
    <th align="center">Profile</th>
  </tr>

  <tr>
    <td align="center">
      <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/github.svg" width="35"/>
      <br/>GitHub
    </td>
    <td align="center">
      <a href="https://github.com/shriram7057" target="_blank">
        https://github.com/shriram7057
      </a>
    </td>
  </tr>

  <tr>
    <td align="center">
      <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" width="35"/>
      <br/>LinkedIn
    </td>
    <td align="center">
      <a href="https://linkedin.com/in/Shriram%20Lahane" target="_blank">
        https://linkedin.com/in/Shriram Lahane
      </a>
    </td>
  </tr>

  <tr>
    <td align="center">
      <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/kaggle.svg" width="35"/>
      <br/>Kaggle
    </td>
    <td align="center">
      <a href="https://kaggle.com/shriramlahane" target="_blank">
        https://kaggle.com/shriramlahane
      </a>
    </td>
  </tr>

  <tr>
    <td align="center">
      <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/instagram.svg" width="35"/>
      <br/>Instagram
    </td>
    <td align="center">
      <a href="https://instagram.com/pvt.shree_01" target="_blank">
        https://instagram.com/pvt.shree_01
      </a>
    </td>
  </tr>

  <tr>
    <td align="center">
      <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/leet-code.svg" width="35"/>
      <br/>LeetCode
    </td>
    <td align="center">
      <a href="https://leetcode.com/shriram-01" target="_blank">
        https://leetcode.com/shriram-01
      </a>
    </td>
  </tr>

  <tr>
    <td align="center">
      <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/geeks-for-geeks.svg" width="70"/>
      <br/>GeeksforGeeks
    </td>
    <td align="center">
      <a href="https://auth.geeksforgeeks.org/user/shriram01" target="_blank">
        https://auth.geeksforgeeks.org/user/shriram01
      </a>
    </td>
  </tr>
</table>
